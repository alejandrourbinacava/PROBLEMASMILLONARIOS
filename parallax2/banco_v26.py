...
("CAP4_SOCIO", 140, [
 ...
 C("Leelo otra vez.", 3, estruct="semaforo", g=dict(tipo="titular", lineas=["*Tres veces* mas."]),
   e="maquina"),
...
def separar_texto(L):
    ...
    for c in L:
        ...
        if max((solapa(r, m) for m in imgs), default=0) <= 0.15:
            continue
        w = c["caja"].get("w", 0.4); h = c["caja"].get("h", 0.16)
        mejor, mejor_v = None, 1.0
        for cx in (0.26, 0.50, 0.74):
            for cy in (0.16, 0.28, 0.42, 0.74):
                cand = (cx - w / 2, cy - h / 2, cx + w / 2, cy + h / 2)
                v = max((solapa(cand, m) for m in imgs), default=0)
                # penaliza salirse del lienzo
                v += 0.5 * (max(0, -cand[0]) + max(0, cand[2] - 1))
                if v < mejor_v:
                    mejor, mejor_v = (cx, cy), v
        if mejor:
            c["caja"]["x"], c["caja"]["y"] = mejor
            c["caja"]["anclaje"] = "centro"
            c["reubicado"] = True
    return L
...
def pila_arq(capas, graf, ent, i, cap, arq, texto=""):
    ...
    if "titular" in H:
        ...
        if cj.get("h", 0.2) < 0.18 and est["px_rel"] > 0.062:
            est = dict(TIPO["titular"])
        cj.update(est)
        frase = recortar_frase(texto, est["max_chars"])
        L.append({"rol": "titular", "tipo_capa": "codigo", "hueco": "titular",
                  "forma": "frase", "texto": frase, "caja": cj,
    ...
    L = separar_texto(L)
    while len(L) < 5:
        L.append({"rol": "acabado", "tipo_capa": "codigo",
...
ARQUETIPOS = {
 ...
 "contraste": dict(
   simetria="espejo",
   huecos={
     "izq_sujeto": dict(x=0.22, y=0.90, h=0.66, anclaje="abajo"),
     "izq_base":   dict(x=0.20, y=1.01, w=0.30, h=0.24, anclaje="abajo"),
     "der_sujeto": dict(x=0.78, y=0.90, h=0.66, anclaje="abajo"),
     "der_base":   dict(x=0.80, y=1.01, w=0.32, h=0.26, anclaje="abajo"),
 ...
 "detras": dict(
   simetria="centrada",
   huecos={
     "sujeto":    dict(x=0.54, y=0.86, h=0.74, anclaje="abajo"),
     "estructura":dict(x=0.46, y=1.02, w=0.76, h=0.42, anclaje="abajo"),
     "marca_a":   dict(x=0.22, y=0.13, w=0.20, anclaje="centro"),
     "titular":   dict(x=0.78, y=0.20, w=0.34, anclaje="centro"),
   }),
 ...
 "sobre": dict(
   simetria="asimetrica",
   huecos={
     "superficie":dict(x=0.50, y=1.01, w=1.12, h=0.20, anclaje="abajo"),
     "objeto":    dict(x=0.42, y=0.87, w=0.66, h=0.34, anclaje="abajo"),
 ...
 "escala": dict(
   simetria="asimetrica",
   huecos={
     "pequeno":   dict(x=0.26, y=0.93, h=0.16, anclaje="abajo"),
     "grande":    dict(x=0.72, y=0.97, h=0.62, anclaje="abajo"),
 ...
 "cadena": dict(
   simetria="rejilla",
   huecos={
     "item_1":    dict(x=0.20, y=0.94, h=0.34, anclaje="abajo"),
     "item_2":    dict(x=0.50, y=0.94, h=0.34, anclaje="abajo"),
     "item_3":    dict(x=0.80, y=0.94, h=0.34, anclaje="abajo"),
 ...
 "unico": dict(
   simetria="centrada",
   huecos={
     "objeto":    dict(x=0.50, y=0.96, h=0.56, anclaje="abajo"),
...
def trocear(dur, tope=5.0, minimo=3.0):
    """Parte una idea en planos de 3 a 5 s. El dinamismo sale de cortar."""
    k = max(1, int(math.ceil(dur / tope)))
    while dur / k < minimo and k > 1:
        k -= 1
    paso = dur / k
    return [round(paso * 2) / 2 for _ in range(k)]
