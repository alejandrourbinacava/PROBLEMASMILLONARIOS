...
def guion_estados(arq, capas, graf, texto, dur):
    """
    Devuelve la coreografia: en que momento entra cada elemento, adonde se
    mueve el anterior para hacerle sitio y cuando sale.
    """
    n = max(2, min(7, int(round(dur / PASO_ESTADO)) + 1))
    H = ARQUETIPOS[arq]["huecos"]
    pares = repartir(arq, capas)
    tiene_dato = bool(graf) and "dato" in H
    tiene_txt = "titular" in H

    # orden de aparicion: primero la imagen que manda, luego el resto,
    # luego el dato y por ultimo la frase
    guion = [c for c, _ in pares]
    if tiene_dato:
        guion.append("@dato")
    if tiene_txt:
        guion.append("@titular")
    ...
    if guion and guion[0] not in ("@dato", "@titular"):
        guion.insert(1, "@asienta")            # el primero se afianza
    if tiene_txt and len(texto) > 30:
        guion.append("@titular_2")             # la frase entra en dos lineas
    guion = guion[:n] or ["@titular"]
    ...
    for k, nuevo in enumerate(guion):
        ...
        for ref in en_escena:
            if ref in ("@dato",):
                caja = dict(H["dato"])
            elif ref in ("@titular", "@titular_2"):
                caja = dict(H["titular"])
            elif ref == "@asienta":
                continue                        # no es un elemento, es un gesto
            else:
                caja = dict(H[dict(pares)[ref]])
            # el elemento que dominaba cede sitio al que entra: encoge y se
            # aparta hacia su lado
            if ref == "@asienta":
                continue
            if ref != nuevo and ref == guion[0] and len(en_escena) > 2:
                caja["w"] = round(caja.get("w", 0.5) * 0.72, 3)
                caja["h"] = round(caja.get("h", 0.4) * 0.72, 3)
                caja["x"] = round(0.5 + (caja["x"] - 0.5) * 1.45, 3)
            e["elementos"].append({"ref": ref, "caja": caja})
        estados.append(e)

    # la frase se escribe en dos tiempos: primera mitad y frase completa
    if "@titular_2" in guion:
        mitad = " ".join(texto.split()[: max(2, len(texto.split()) // 2)])
        for e in estados:
            for x in e["elementos"]:
                if x["ref"] == "@titular":
                    x["texto"] = mitad if e["entra"] != "@titular_2" else texto
                if x["ref"] == "@titular_2":
                    x["ref"] = "@titular"; x["texto"] = texto
        for e in estados:
            vistos_ = set(); nuevos = []
            for x in e["elementos"]:
                if x["ref"] in vistos_:
                    continue
                vistos_.add(x["ref"]); nuevos.append(x)
            e["elementos"] = nuevos
    ...
    if len(estados) >= 4 and len(guion) > 2:
        ult = dict(estados[-1])
        ult["sale"] = guion[0]
        ult["elementos"] = [x for x in ult["elementos"] if x["ref"] != guion[0]]
        ...
        for i, (txt, dur, tipo, capas, graf, ent, trans) in enumerate(beats, 1):
            ...
            for j, d in enumerate(trozos):
                ...
                lista = pila_arq(cap_esc, g, e, len(escenas), cap, arq,
                                 frases[j])
                enc = arq
                estados = guion_estados(arq, cap_esc, g, frases[j], d)
