...
def plegar_tiempo(L, dur, texto):
    """
    Mete la coreografia DENTRO de cada capa y elimina la lista de estados.

    El problema de tener `capas` y `estados` por separado es que obliga a
    cruzarlas por id: el texto vivia en la capa y la caja en el estado, y
    si el renderizador dibuja desde el estado, el texto le llega vacio y la
    escena sale en blanco. Con esto cada capa se describe entera a lo largo
    del tiempo y no hay nada que cruzar.

        desde / hasta   cuando aparece y cuando se va
        caja            donde esta al aparecer
        movimientos     [{t, caja}] adonde se mueve despues

    Una sola lista. Imposible de empalmar mal.
    """
    est = guion_estados(L, dur, texto)
    ...
    for st in est:
        for x in st["elementos"]:
            ...
            if st["entra"] == x["ref"]:
                ...
                mov = {"t": st["t"], "caja": dict(x["caja"])}
                if "texto" in x and x["texto"] != c.get("texto"):
                    mov["texto"] = x["texto"]
                if mov not in c["movimientos"]:
                    c["movimientos"].append(mov)
        if st.get("sale"):
            por_id[st["sale"]]["hasta"] = st["t"]

    for c in L:
        c["deriva"] = {"dx": round(DERIVA * (1 if int(c["id"][1:]) % 2 else -1), 4),
...
def main():
        ...
        for i, (txt, dur, tipo, capas, graf, ent, trans) in enumerate(beats, 1):
            ...
            for j, d in enumerate(trozos):
                ...
                lista = pila_arq(cap_esc, g, e, len(escenas), cap, arq,
                                 frases[j])
                enc = arq
                lista = plegar_tiempo(lista, d, frases[j])
                esc = {"id": ident, "texto": txt, "frase": frases[j],
                       "duracion": d,
                       "n_momentos": 1 + sum(len(c["movimientos"])
                                             for c in lista)
                       + sum(1 for c in lista if c["desde"] > 0),
                       "arquetipo": arq,
    ...
    for e in escenas:
        ...
        for c in cs:
            if c.get("hasta", 1) <= c.get("desde", 0):
                fallos.append(f'{e["id"]}: {c["id"]} se va antes de aparecer')
            if c["rol"] == "titular" and not c.get("texto"):
                fallos.append(f'{e["id"]}: {c["rol"]} sin texto')
            for m in c.get("movimientos", []):
                if c["rol"] in ("titular", "grafico") and "px_rel" not in m["caja"]:
                    fallos.append(f'{e["id"]}: movimiento de texto sin tamano')
    ...
    guion = {"lienzo": {"w": 1920, "h": 1080, "fps": 25, "ppm": 140},
             ...
             "como_leer": [
               "HAY UNA SOLA LISTA: capas[]. No hay nada que cruzar.",
               "",
               "Cada capa se describe entera:",
               "  archivo / forma  que se dibuja",
               "  texto            si es tipografia, la cadena EXACTA",
               "  caja             x, y, w, h en fracciones del lienzo",
               "                   (0.5 = centro). Si es texto, ademas",
               "                   px_rel, lineas y max_chars.",
               "  desde / hasta    segundos dentro de la escena",
               "  movimientos      [{t, caja}] adonde se mueve despues",
               "  deriva           dx, dy por segundo (movimiento continuo)",
               "",
               "Se dibujan TODAS las capas cuyo desde <= t < hasta, en el",
               "orden del array. Nada mas.",
               "",
               "Cada escena es UN SOLO PLANO: entre movimientos NO se corta,",
               "la capa se interpola de una caja a la siguiente.",
               "",
               "px_rel es la altura de la fuente como fraccion del alto del",
               "lienzo: px_rel 0.09 a 1080 son 97 px.",
               "Los *asteriscos* del texto marcan la palabra de acento."
             ],
             "escenas": escenas}
    ...
    est = [e["n_momentos"] for e in escenas]
    print(f"momentos por escena: min {min(est)}  max {max(est)}  "
